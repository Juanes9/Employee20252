...
namespace Employee.Frontend.Components.Pages.Auth
{
    ...
    public partial class EditUser
    {
        ...
        /*protected override async Task OnInitializedAsync()
        {
            ...
            await LoadStatesAsyn(user!.City!.State!.Country!.Id);//resolver
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var countryId = user?.City?.State?.CountryId;
            var stateId = user?.City?.State?.Id;

            if (countryId is not null)
            {
                await LoadStatesAsyn(countryId.Value);
                selectedCountry = countries?.FirstOrDefault(c => c.Id == countryId.Value) ?? new Country();
            }
            else
            {
                states = null;
            ...
            {
                await LoadCitiesAsyn(stateId.Value);
                selectedState = user?.City?.State ?? new State();
            }
            else
            {
                cities = null;
            ...
            selectedCity = user?.City ?? new City();
