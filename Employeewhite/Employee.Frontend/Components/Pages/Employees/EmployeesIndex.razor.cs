...
{
    private List<EmployeeBD>? Employees { get; set; }
    private MudTable<EmployeeBD> table = new();
    ...
    private async Task<TableData<EmployeeBD>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
        ...
        var responseHttp = await Repository.GetAsync<List<EmployeeBD>>(url);
        if (responseHttp.Error)
        {
            var message = await responseHttp.GetErrorMessageAsync();
            Snackbar.Add(message!, Severity.Error);
            return new TableData<EmployeeBD> { Items = [], TotalItems = 0 };
        }
        if (responseHttp.Response == null)
        {
            return new TableData<EmployeeBD> { Items = [], TotalItems = 0 };
        }
        return new TableData<EmployeeBD>
        {
    ...
    private async Task DeleteAsync(EmployeeBD employee)
    {
        var parameters = new DialogParameters
        {
